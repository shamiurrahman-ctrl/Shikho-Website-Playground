...
/**
 * Self-contained pinned, scroll-driven hero + 6-feature scrollytelling.
 * All animation is scoped to this component's own ref, so the section can be
 * instantiated more than once on a page without id collisions driving the wrong
 * copy. (Header-tone + stats logic lives in the parent; the guarded blocks below
 * simply no-op here because those elements aren't in this subtree.)
 */
...
<!-- ============ PINNED SCROLL TRACK ============ -->
  <section id="track" style="position:relative;height:900vh;">
    ...
    <div id="stage" style="position:sticky;top:0;height:100vh;overflow:hidden;background:linear-gradient(180deg,#CADFF4 0%,#DCE9F7 32%,#EFE9F0 58%,#FBEEDB 80%,#FEF6EC 100%);">
      ...
      <!-- cloud-window scene (shown only in the "window" variant, replaces girl+phone+orbit) -->
      <img id="heroWindow" src="/assets/hero-window.png" alt="" style="position:absolute;right:2%;top:50%;transform:translateY(-50%);height:92vh;width:auto;max-width:50%;object-fit:contain;z-index:24;pointer-events:none;display:none;will-change:transform;filter:drop-shadow(0 30px 60px rgba(13,26,91,.18));">
...
export default function HeroSection({ variant = 'default' }: { variant?: 'default' | 'window' }) {
  ...
  useEffect(() => {
    ...
    const el = {
      ...
      statsHead: q('#statsHead'),
      statsGroup: q('#statsGroup'),
      scards: qa('.scard'),
    };
    ...
    const render = (now: number) => {
      ...
      // ---- STATS: scroll-driven cascading cards (own pinned track) ----
      if (el.statsTrack && el.statsGroup) {
        const vw = window.innerWidth;
        const st = el.statsTrack.getBoundingClientRect();
        const sTravel = el.statsTrack.offsetHeight - vh;
        const sp = clamp(-st.top / sTravel, 0, 1);
        // fit the 1008x522 cascade into the viewport
        const gscale = clamp(Math.min((vw - 80) / 1278, (vh - 200) / 468), 0.4, 1);
        el.statsGroup.style.transform = `translate(-50%,-50%) scale(${gscale})`;
        if (el.statsHead) {
          const h = mc(sp, 0.0, 0.14);
          el.statsHead.style.opacity = String(h);
          el.statsHead.style.transform = `translateY(${lerp(28, 0, eo(h))}px)`;
        }
        el.scards.forEach((c, i) => {
          const t = eo(clamp((sp - 0.06 - i * 0.11) / 0.44, 0, 1));
    ...
    // variant: "window" swaps the girl + phone + orbiting icons for the cloud-window scene
    const variant = root.getAttribute('data-variant') || 'default';
    if (variant === 'window') {
      [el.student, el.phone, el.orbit].forEach((e) => {
        if (e) (e as HTMLElement).style.display = 'none';
      });
      const heroWindow = q('#heroWindow');
  ...
  return (
    <div
      ...
      dangerouslySetInnerHTML={{ __html: HERO_MARKUP }}
    />
  );
}
